...
DRAM *dram_new()
{
    ...
    DRAM *dram = (DRAM*) calloc(NUM_BANKS, sizeof(DRAM));

    for (unsigned int i = 0; i < NUM_BANKS; ++i)
    {
        dram[i].row_buffer.valid = false;
        dram[i].row_buffer.row_id = 0;
    }
...
uint64_t dram_access(DRAM *dram, uint64_t line_addr, bool is_dram_write)
{
    ...
    else if (SIM_MODE == SIM_MODE_DEF || SIM_MODE == SIM_MODE_C)
                               ...
                               bool is_dram_write)
{
    ...
    uint64_t full_address = line_addr << int(log2(CACHE_LINESIZE));
    uint64_t RowPlusBank = full_address >> int(log2(ROW_BUFFER_SIZE));

    unsigned int bank = RowPlusBank % NUM_BANKS;
    uint64_t row_id = RowPlusBank / NUM_BANKS;


    if (DRAM_PAGE_POLICY == OPEN_PAGE)
    {
        // hit
        if (dram[bank].row_buffer.valid && (dram[bank].row_buffer.row_id == row_id))
        {
            delay = DELAY_CAS + DELAY_BUS;
        }

        // miss
        else if (dram[bank].row_buffer.valid && (dram[bank].row_buffer.row_id != row_id))
        {
            delay = DELAY_PRE + DELAY_ACT + DELAY_CAS + DELAY_BUS;
            dram[bank].row_buffer.row_id = row_id;
        }

        // empty
        else if (!dram[bank].row_buffer.valid)
        {
            delay = DELAY_ACT + DELAY_CAS + DELAY_BUS;
            dram[bank].row_buffer.valid = true;
            dram[bank].row_buffer.row_id = row_id;
        }
    }
