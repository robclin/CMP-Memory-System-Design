...
unsigned long long L2_access[2];
unsigned long long L2_hit[2];
                       ...
                       unsigned int core_id)
{
    ...
    #ifdef DEBUG
        printf("Accessing memory in mode DEF (line_addr: 2102327, AccessType: 0, core_id: 0)\n",);
    #endif
    ...
    {
        #ifdef DEBUG
            printf("Accessing memory in mode DEF (line_addr: %lu, AccessType: %lu, core_id: %lu)\n", line_addr, type, core_id);
        #endif
                              ...
                              AccessType type, unsigned int core_id)
{
    ...
    if (type == ACCESS_TYPE_IFETCH) 
    {
        ...
        {   
            // update delay
            delay = memsys_l2_access(sys, line_addr, false, core_id) + 1;

            // install line into corresponding L1 cache
            cache_install(sys->icache, line_addr, false, core_id);

            // if last evicted line is valid and dirty, write it back to rest of the memory system
            if (sys->icache->last_evicted_line.valid && sys->icache->last_evicted_line.dirty)
            {
                // evicted line addresss = {its own tag, index of origenal access}
                line_addr = (sys->icache->last_evicted_line.tag << int(log2(sys->icache->sets))) + (line_addr % sys->icache->sets);
                memsys_l2_access(sys, line_addr, true, core_id);
            }
        }
    }
    ...
    if (type == ACCESS_TYPE_LOAD) 
    {
        ...
        {   
            ...
            {
                // evicted line addresss = {its own tag, index of origenal access}
                line_addr = (sys->dcache->last_evicted_line.tag << int(log2(sys->dcache->sets))) + (line_addr % sys->dcache->sets);
                memsys_l2_access(sys, line_addr, true, core_id);
    ...
    if (type == ACCESS_TYPE_STORE) 
    {
        ...
        else
        {
            ...
            // if last evicted line is valid and dirty, write it back to L2
            if (sys->dcache->last_evicted_line.valid && sys->dcache->last_evicted_line.dirty)
            {
                // evicted line addresss = {its own tag, index of origenal access}
                line_addr = (sys->dcache->last_evicted_line.tag << int(log2(sys->dcache->sets))) + (line_addr % sys->dcache->sets);
                          ...
                          bool is_writeback, unsigned int core_id)
{
    ...
    if (cache_access(sys->l2cache, line_addr, is_writeback, core_id))
    {
        L2_hit[core_id]++;
        return delay;
    }
    else
    {
        ...
        // if last evicted line is valid and dirty, write it back to DRAM
        if (sys->l2cache->last_evicted_line.valid && sys->l2cache->last_evicted_line.dirty)
        {
            // evicted line addresss = {its own tag, index of origenal access}
            line_addr = (sys->l2cache->last_evicted_line.tag << int(log2(sys->l2cache->sets))) + (line_addr % sys->l2cache->sets);
            dram_access(sys->dram, line_addr, true);
        }
    }
                               ...
                               AccessType type, unsigned int core_id)
{
    ...
    // multiply by cache line size to get virtual address in terms of bytes
    uint64_t v_byte_addr = v_line_addr << int(log2(CACHE_LINESIZE));

    // Split into VPN and page offset according to PAGE_SIZE
    uint64_t vpn = v_byte_addr / PAGE_SIZE;
    uint64_t page_offset = v_byte_addr % PAGE_SIZE;

    // resemble PFN with page offset
    uint64_t p_byte_addr = (memsys_convert_vpn_to_pfn(sys, vpn, core_id) << int(log2(PAGE_SIZE))) + page_offset;
    uint64_t p_line_addr = p_byte_addr >> int(log2(CACHE_LINESIZE));
    ...
    if (type == ACCESS_TYPE_IFETCH) 
    {
        ...
        // L1 hit
        if (cache_access(sys->icache_coreid[core_id], p_line_addr, false, core_id)) 
        {
            delay = 1;
        }
        ...
        {   
            // update delay
            delay = memsys_l2_access(sys, p_line_addr, false, core_id) + 1;

            // install line into corresponding L1 cache
            cache_install(sys->icache_coreid[core_id], p_line_addr, false, core_id);

            // if last evicted line is valid and dirty, write it back to rest of the memory system
            if (sys->icache_coreid[core_id]->last_evicted_line.valid && sys->icache_coreid[core_id]->last_evicted_line.dirty)
            {
                // evicted line addresss = {its own tag, index of origenal access}
                p_line_addr = (sys->icache_coreid[core_id]->last_evicted_line.tag << int(log2(sys->icache_coreid[core_id]->sets))) + (p_line_addr % sys->icache_coreid[core_id]->sets);
                memsys_l2_access(sys, p_line_addr, true, core_id);
            }
        }
    }
    ...
    if (type == ACCESS_TYPE_LOAD) 
    {
        ...
        // L1 hit
        if (cache_access(sys->dcache_coreid[core_id], p_line_addr, false, core_id)) 
        {
            delay = 1;  
        }
        ...
        {   
            ...
            {
                // evicted line addresss = {its own tag, index of origenal access}
                p_line_addr = (sys->dcache_coreid[core_id]->last_evicted_line.tag << int(log2(sys->dcache_coreid[core_id]->sets))) + (p_line_addr % sys->dcache_coreid[core_id]->sets);
                memsys_l2_access(sys, p_line_addr, true, core_id);
    ...
    if (type == ACCESS_TYPE_STORE) 
    {
        ...
        // L1 hit
        if (cache_access(sys->dcache_coreid[core_id], p_line_addr, true, core_id)) 
        {
            delay = 1;  
        }
